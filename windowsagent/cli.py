...
@click.option(
    "--scheme",
    type=click.Choice(["default", "high-contrast", "monochrome"]),
    default="default",
    show_default=True,
    help="Colour scheme preset",
)
def overlay_cmd(window: str, refresh: int, scheme: str) -> None:
    ...
    scheme_map = {
        ...
        "high-contrast": high_contrast_scheme,
        "monochrome": monochrome_scheme,
    }

    overlay = OverlayWindow(target_window=window, refresh_ms=refresh)
    overlay.scheme = scheme_map[scheme]()
    click.echo(f"Launching overlay for '{window}' (scheme={scheme}, refresh={refresh}ms)")
